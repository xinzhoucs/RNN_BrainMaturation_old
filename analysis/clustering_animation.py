...
class Analysis(object):
    def __init__(self, model_dir, data_type, res, normalization_method='max'):# add res by yichen
    ...
    def plot_cluster_score(self, save_name=None):
        ...
        #ax.set_ylim([0, 0.32])
        ax.set_ylim([0, 0.8])# modified by yichen
        if save:
    ...
    def plot_variance(self, pic_num, growth):
        labels = self.labels
        ######################### Plotting Variance ###################################
        ...
        if self.data_type == 'rule':
            ...
            tick_names = [rule_name[r]+'('+growth[r]+')' for r in self.rules]
            fs = 10 # yichen
            labelpad = 15 # yichen
        ...
        if save:
            plt.savefig('variance_frame/'+str(pic_num)+'.jpg', transparent=False)# modified by yichen
            plt.close()
        #plt.show()
    ...
    def plot_example_unit(self):
        ...
        if self.data_type == 'rule':
            ...
            pretty_singleneuron_plot(self.model_dir, ['overlap'], [self.ind_active[ind]],
                                     epoch=None, save=save, ylabel_firstonly=True)#add by yichen
