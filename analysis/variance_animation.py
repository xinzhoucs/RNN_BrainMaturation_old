...
def _compute_variance(model, sess, rules=None, random_rotation=False): #add sess by yichen model_dir>model
    """Compute variance for all tasks.
    ...
    # add by yichen start ==================================================================
    #model.restore()
    result = _compute_variance_bymodel(model, sess, rules, random_rotation)
    return result
    # add by yichen end ====================================================================
...
def _compute_hist_varprop(model, sess, rule_pair, random_rotation=False): #add sess by yichen model_dir>model
    data_type = 'rule'
    ...
    res = _compute_variance(model, sess, rules=None, random_rotation=random_rotation)# add by yichen
    ...
    return hist, bins_edge, res


def compute_hist_varprop(model, sess, rule_pair, random_rotation=False): # add sess by yichen model_dir>model
    data_type = 'rule'
    assert len(rule_pair) == 2
    assert data_type == 'rule'
    ...
    '''for model_dir in model_dirs:
        ...
        hists.append(hist)'''
    # add by yichen start ==========================================================================
    hist, bins_edge_, res = _compute_hist_varprop(model, sess, rule_pair, random_rotation)
    if hist is None:
        pass
    else:
        bins_edge = bins_edge_
...
def plot_hist_varprop_selection(model_dir, figname_extra=None):
    ...
    rule_pair_list = [('overlap','zero_gap'),('overlap','gap'),('zero_gap','gap'),('odrd','overlap')] #add by yichen
...
def plot_hist_varprop_all(model, sess, pic_num, growth, plot_control=True): #add sess,growth by yichen model_dir>model
    '''
    Plot histogram of proportion of variance for some tasks across units
    :param save_name:
    :param data_type:
    ...
    hp = model.hp # add by yichen
    rules = hp['rules']

    figsize = (12, 12)# yichen
    ...
    for i in range(len(rules)):
        for j in range(len(rules)):
            ax = axarr[i, j]
            if i == 0:
                ax.set_title(rule_name[rules[j]]+'('+growth[rules[j]]+')', fontsize=fs, rotation=45, va='bottom')# yichen
            if j == 0:
                ax.set_ylabel(rule_name[rules[i]]+'('+growth[rules[i]]+')', fontsize=fs, rotation=45, ha='right')# yichen
            ...
            hists, bins_edge,res = compute_hist_varprop(model, sess, (rules[i], rules[j]))# yichen
            hist_low, hist_med, hist_high = np.percentile(hists, [10, 50, 90], axis=0)
            hist_med /= hist_med.sum()

            # Control case
            if plot_control:
                hists_ctrl, _ , _= compute_hist_varprop(model, sess, (rules[i], rules[j]), random_rotation=True)#yichen
    ...
    # plt.tight_layout()
    plt.savefig('FTV_frame/'+str(pic_num)+'.jpg', transparent=False) #modified by yichen
    plt.close()
    return res # add by yichen
