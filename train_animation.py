...
from analysis import clustering_animation
from analysis import variance_animation
...
def do_eval(sess, model, log, rule_train):
    ...
    for rule_test in hp['rules']:
        ...
        elif hp['infancy_target_perf']<np.mean(perf_tmp)<=hp['young_target_perf']:
            rule_grow[rule_test] = 'I=>Y'
        elif hp['young_target_perf']<np.mean(perf_tmp)<=hp['adult_target_perf']:
        ...
        while step * hp['batch_size_train'] <= max_steps:
            try:
                ...
                if step % display_step == 0:
                    ...
                    log, growth = do_eval(sess, model, log, hp['rule_trains'])
                    #if log['perf_avg'][-1] > model.hp['target_perf']:
                    #check if minimum performance is above target 
                    #####################################plot frams ################################################
                    #add by yichen
                    res = variance_animation.plot_hist_varprop_all(model, sess, step, growth, True)
                    CA = clustering_animation.Analysis(model_dir,'rule',res)
                    CA.plot_variance(step, growth)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modeldir', type=str, default='data/ALLNEW256_fuse_onehot_input_odrdrandom')#add by yichen
    ...
    train(args.modeldir,
        ...
        display_step=10)#modified by yichen 5=>10
