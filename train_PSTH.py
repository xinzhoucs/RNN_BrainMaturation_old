...
def train(model_dir,
    ...
    Returns:
        model is stored at model_dir/trail_number/model.ckpt
    ...
    tools.save_hp(hp, model_dir)# modified by yichen model->root

    # Build the model
    model = Model(model_dir, hp=hp)# modified by yichen model->root
    ...
    log['model_dir'] = model_dir#modified by yichen model->root
        ...
        while step * hp['batch_size_train'] <= max_steps:
            try:
                ...
                if step % display_step == 0:
                    trail_number = step * hp['batch_size_train']# add by yichen
                    log['trials'].append(trail_number)
                    tools.mkdir_p(model_dir+'/'+str(trail_number))# add by yichen
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modeldir', type=str, default='data/odr')#add by yichen
